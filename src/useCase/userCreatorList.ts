...
import { listActions } from 'src/stores/domain/user/userCreator';
...
interface FetchResources extends RequestParams, SearchParams.UserCreatorList {}
...
export const fetchResources = (args: FetchResources): Action => {
  return async (dispatch) => {
    try {
      ...
      const res = await userRequest.getCreatorList({ ...params });
